...
<head>
    <title>Aktif Siparişler Admin Paneli - GameSatış</title>
...
if(isset($_GET['delete'])) {
	...
	// Sipariş durumunu güncelle ve oyun miktarını azalt
if(isset($_POST['update_status'])) {
    ...
    try {
        ...
        // Durum "Gönderildi" olarak değiştirilirse ilgili oyunun miktarını 1 azaltın
        if($status == 'Gönderildi') {
            // Siparişten oyun kimliğini ve miktarını al
            $sql = "SELECT game_id, quantity FROM orders WHERE id=$id";
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_assoc($result);
            $game_id = $row['game_id'];
         

            // Oyunun miktarını azaltın
            $sql = "UPDATE games SET quantity=quantity-1 WHERE id=$game_id";
            mysqli_query($conn, $sql);
        }
	...
	$sql = "SELECT id, user_id,username, email, game_id,game_name,game_price,card_no, card_name, exp_date, cvc, date,last_edit, status, quantity FROM orders WHERE status='Beklemede'";
